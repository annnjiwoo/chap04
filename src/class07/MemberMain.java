...
public class MemberMain {
	public static void main(String[] args) {
		...
		//[문제.1] 20대 후반 여성의 총 방문 횟수 구해서 반환해주는 메소드 만들기
		int count = getVissitCount(members);
		System.out.println("20대 후반 여성의 총 방문 횟수 : " + count);
		System.out.println();
		//[문제.2] 40대이상의 총 인원 수를 구해주는 메소드 만들기
		count = getCount40(members);
		System.out.println("40대이상의 총 인원 수 : " + count);
		
	
	}
	//[문제.2] 40대이상의 총 인원 수를 구해주는 메소드 출력
	...
	//[문제.1] 20대 후반 여성의 총 방문 횟수 구해서 반환해주는 메소드 만들기
	private static int getVissitCount(Member[] members) {
		int count = 0;
		for (Member member : members) {
	...
	private static void printMemberList(Member[] members) {
		for (int i = 0; i < members.length; i++) {
			System.out.println(members[i].getName() + "\t " + members[i].getJoinDate()  + "\t "
						+ members[i].getLastVisitDate()+ "\t " + members[i].getNumVisit() + "\t " +members[i].getGender() 
						+"\t " + members[i].getAge());
			
		}
	}
	// 문자열 배열에 들어있는 값으로 객체 생성해서 members에 저장해주는 메소드 출력
	private static void initData(Member[] members) {
		...
		for (int i = 0; i < member.length; i++) {
			...
			char gender = arr[4].charAt(0);	//charAt : 스트링 타입의 문자열을 char타입으로 한글자만 받음
